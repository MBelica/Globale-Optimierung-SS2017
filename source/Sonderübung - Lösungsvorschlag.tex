...
\begin{enumerate}
	\item Zeigen Sie, dass $P$ lösbar ist.
		\begin{proof}
			...
			$$ \left\| f(x) - f(y) \right\|_2 = \frac{1}{2} \left\| \| x \|^2_2 - \| y \|_2^2 \right\|_2 \leq \frac{1}{2} \| x - y \|_2^2 \leq \epsilon. $$
			Behauptung: die Menge $M$ ist abgeschlossen bzw. das Komplement
			$$ \R^m \setminus M = \{ w \in R^m \colon A w < e \}. $$
			ist offen. Betrachte hierfür $w \in \R^m \setminus M$, dann gilt mit der Norm $\| w \|_{\infty } \coloneqq \max_{i=1,\dotsc ,m}|w_{i}|$ und dem Radius
			$$ \epsilon \coloneqq \frac{1}{2} \| Aw - e \|_\infty > 0, $$ 
			dass alle 
			$$ x \in B_{\epsilon}(w) = \{ x \in \R^m \colon \| w - x \| < \epsilon \} $$
			in $M$ liegen, da folgt
			$$ \| e - Ax \|_{\infty} \leq \| e - Aw \|_\infty + \| Aw - Ax \|_\infty < \frac{1}{2} 	\epsilon + \frac{1}{2} \epsilon \Longrightarrow Ax \leq e. $$
			Somit ist $M$ abgeschlossen, woraus folgt, dass die Funktion $f$ koerziv ist, denn nach Definition 1.2.37 müssen nur alle Folgen $(x^\nu) \subseteq M$ mit $\| x^\nu \| \rightarrow \infty$ betrachtet werden und für die gilt mit der Äquivalenz der Normen im $\R^m$:
	...
	\item Formulieren Sie, ausgeheen von Aufgabenteil b), ein duales Optimierungsproblem, das nur von der Dualvariabel $\lambda$ abhängt.
\end{enumerate}
