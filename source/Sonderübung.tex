...
\begin{enumerate}
	\item $M \colon \big\{ x \in \R^2 \colon \left( x_1 - 1 \right)^2 + \left( x_2 - 1 \right)^2 \leq 4 \big\}$
		\begin{proof}
			...
			$$ M = f_{\leq}^{0}. $$
			Da $f$ als Polynom zweimal stetig differenzierbar ist, können wir die Hesse-Matrix aufstellen:
			$$ \nabla f(x) = \left(\begin{array}{c} 2x_1 - 2 \\ 2 x_2 -2 \end{array}\right) \Rightarrow D^2 f(x) =\left(\begin{array}{rr} 2 &  0 \\
     0 & 2 \end{array}\right) . $$
     Die Eigenwerte erhalten wir durch die charakteristische Gleichung 
     	$$\det (D^2 f(x) -  \lambda I) = (2-\lambda)^2 \overset{!}{=} 0.$$
     Das heißt $\lambda = 2$ ist zweifacher Eigenwert und $f$ ist somit konvex. Nach Übung 4.4 ist $M$ damit eine konvexe Menge.
			\begin{figure}[h!] \centering
