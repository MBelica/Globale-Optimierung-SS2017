...
\begin{itemize}
	\item Aufgabe 3: d)
	\item Aufgabe 4: Scheint mir nicht genügend zu sein! Ist das echt ein lineares Problem?
	\item Aufgabe 5: Konvexität zeigen, f'' > 0 ?
\end{itemize}
...
\begin{proof} Nach Vorlesung (Definition 1.2.37) gilt: ~\\ ~\medskip
	...
	\begin{enumerate}
		\item $f(x) = - x^5$, $M =(- \infty, 1)$: ~\medskip
			...
			Da das Infimum aber nicht in der Menge angenommen wird ($f(x) > -1$ für alle $x \in M$), ist das Problem nach Definition 1.2.3 nicht lösbar. ~\\
		\item  $f(x) = 9 x_1^2 - 6 x_1 x_2^2 + x_2^4$, $M = \R^2$: ~\medskip
			...
			Da es $(x_1, x_2) \in M$ gibt, die die obige Bedingung erfüllen, nimmt $f$ auf $M$ sein Infimum an, und das Problem ist nach Definition 1.2.3. lösbar.
		\item $f(x) = \frac{x^T A x}{\| x - b\|_2 + 1}$, mit $A \in \R^{n \times n}$ positiv definit, $b \in \R^n$ und $M = \R^n$: ~\medskip
			...
			Da $M$ nicht-leer und abegschlossen, $f$ stetig und koerziv ist, ist das Problem nach Korollar 1.2.30 lösbar.
	\end{enumerate}
\end{proof}
...
\begin{enumerate}
	\item Geben Sie die verallgemeinerte Epigraph-Umformulierung $P_{epi}$ von $P$ an (siehe Übung 1.3.9. im Skript). Begründen Sie, welche Funktionen $f$, $g$, $F$ und $G$ Sie für die Umformulierung verwenden.
		\begin{proof}
			...
			$$ P_{epi}: \min_{(x,Â \alpha, \beta) \in \R^2 \times R \times \R} F(\alpha) \text{ s.t. } G(\beta) \leq 0, f(x) \leq \alpha, g(x) \leq \beta, x \in X $$
			$$ \iff \min_{(x,Â \alpha) \in \R^2 \times R} e^{-\alpha} \text{ s.t. } \min\big\{- x_1 - 3, -\left|x_2 - 4\right|, x_1 + x_2 - 20\big\} \leq \alpha, x \in X $$
		\end{proof}
	\item Formulieren Sie, aufbauend auf Aufgabenteil a), ein lineares Optimierungsproblem $P_{lin}$, welches die selben Optimalpunkte wie $P_{epi}$ besitzt.
		\begin{proof}
			...
			\end{align*} % todo F oben umformulieren
			...
			$$  \tilde{P}_{epi}: \min_{(x,Â \alpha) \in \R^2 \times R} e^{\alpha} \text{ s.t. } x \in X, \begin{cases} x_1 + 3 \leq \alpha \\
			...
			$$  {P}_{lin}: \min_{(x,Â \alpha) \in \R^2 \times R} \alpha \text{ s.t. } x \in X, \begin{cases} x_1 + 3 \leq \alpha \\
			 x_2 - 4 \leq \alpha, ~ x_2 - 4 \geq - \alpha \\ -(x_1 + x_2) + 20 \leq \alpha \end{cases} $$			 
		\end{proof}
		...
		\begin{proof}
			...
			$$\begin{cases} x_1 + 3 \leq \alpha \\
			 ...
			 Die Funktion $f(\alpha) = \alpha$ ist als Identität stetig. Für den verschärften Satz von Weierstraß bleibt zu zeigen, dass für ein $\beta \in \R$ die Menge
			  $$ \operatorname{lev}_{\leq}^{\beta}(f, M)= \big\{ (\alpha, x) \in M | f(\alpha) = \alpha \leq \beta \}  $$
			...
			Somit liefert ein $\beta > \frac{19}{3}$, dass $f(\alpha) = \alpha \in \left[\frac{19}{3}, \beta \right]$ beschränkt ist. Das in Kombination mit $(*)$ bzw. $(**)$ liefert die Beschränktheit von $x_1, x_2$. Zusammengefasst ist für ein $\beta > \frac{19}{3}$
				$$ \big\{ (\alpha, x) \in M | f(\alpha) = \alpha \leq \beta \} $$
...
\begin{proof}
	...
	wobei wir ausnutzen, dass $Ax \leq b \iff Ax -b \leq 0 \iff  \max_i \{ (Ax - b)_i \} \leq 0$. Damit sind die Voraussetzungen der verallgemeinerten Epigraph-Formulierung gegeben und die äquivalente Epigraph-Formulierung lautet:
...
\begin{enumerate}
	\item $M \colon \big\{ x \in \R^2 \colon \left( x_1 - 1 \right)^2 + \left( x_2 - 1 \right)^2 \leq 4 \big\}$
		\begin{proof}
			...
			\end{align*}
			Damit ist unter Verwendung von Cauchy-Schwartz
			\begin{align*}
				(z_1 - 1)^2 + (z_2 - 1)^2 & = (\lambda (x_1 - 1) + (1 - \lambda) (y_1 - 1))^2 + (\lambda (x_2 - 1) + (1 - \lambda) (y_2 - 1))^2 \\
				& \leq 2 (\lambda^2 (x_1 - 1)^2 + (1 - \lambda)^2 (y_1 - 1)^2) + 2 (\lambda^2 (x_2 - 1)^2 + (1 - \lambda)^2 (y_2 - 1)^2) \\
				& = 2 \left( \lambda^2 \left( (x_1 - 1)^2 + (x_2 - 1)^2  \right) + (1 - \lambda)^2 \left( (y_1 - 1)^2 + (y_2 - 1)^2 \right) \right) \\
				& \leq 2 \left( \lambda^2 4 + (1 - \lambda)^2 4 \right) \\
				& = 8 \left( \lambda^2 + (1 - \lambda)^2 \right) \overset{(*)}{\leq} 8 \left( \left( \frac{1}{2} \right)^2 + \left( 1 - \frac{1}{2} \right)^2 \right) = 4
			\end{align*} 
			$(*)$: hier haben wir ausgenutzt, dass die Funktion $g(x) =  x^2 + \left( 1 - x \right)^2$ ihr Maximum bei $\frac{1}{2}$ annimmt. Dazu die FOC:
			$$ g'(x) = 2 x - 2 (1-x) \overset{!}{=} 0 \iff x = \frac{1}{2} $$
			und $g''(x) = 2 + 2 > 0$. Das heißt $z \in M$.
...
Aufgrund der 2. Ungleichung gilt $z \notin M $ und damit ist $M$ nicht konvex.  ~\\
	...
	\item $M \colon \big\{ x \in \R^2 \colon \left( x_1 - 1 \right)^2 + \left( x_2 - 1 \right)^2 \leq 4, \left( x_1 - 3 \right)^2 + \left( x_2 - 1 \right)^2 \geq 16  \big\}$
		\begin{proof}
			...
			$$  \left( x_2 - 1 \right)^2 \leq 4 - \left( x_1 - 1 \right)^2 \text{ und }  \left( x_2 - 1 \right)^2 \geq 16 - \left( x_1 - 3 \right)^2. $$
			Einsetzen ineinander liefert
			$$  4 - \left( x_1 - 1 \right)^2  \geq 16 - \left( x_1 - 3 \right)^2 \iff 8 - 4x_1   \geq 12 \iff x_1 \leq -1 $$
			Da allerdings 
				$$ 4 \geq \left( x_1 - 1 \right)^2 + \left( x_2 - 1 \right)^2 \geq (x_1 - 1)^2 \iff 3 \geq x_1 $$ 
			und $-1 \geq x_1$, folgt $x_1 = -1$. Daraus folgt direkt
			$$  0 \geq + \left( x_2 - 1 \right)^2 \geq 0 \iff x_2 = 1 $$
			und damit die Behauptung.
				\begin{figure}[h!] \centering
  					\includegraphics[scale=0.4]{img/suv-iii}
 					 \label{fig:fig3}
				\end{figure}
		\end{proof}
\end{enumerate}
